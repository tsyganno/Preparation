...
for el in info:
    directory = el.strip('/').split('/')
    indentation = '  ' * (len(directory) - 1)
    if '.' in directory[-1]:
        last_file = zip_file.getinfo(el)
        size_el = convert_bytes(last_file.file_size)
        print(indentation + directory[-1] + ' ' + size_el)
    else:
        print(indentation + directory[-1])
